...
say ""
say "  \"The fourth Lambda is the Reviewer Callback Lambda — it handles POST /review."
say "  The first thing it does is an atomic idempotency check: if a reviewer accidentally"
say "  submits twice, the second call is silently dropped. A patient's record cannot be"
say "  corrupted by a duplicate POST.\""
say ""
say "  \"And the Status Lambda is a simple read-through — GET /status/{id} reads straight"
say "  from DynamoDB. Reads never touch the write path.\""
say ""
